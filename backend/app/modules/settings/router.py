...
def set_config(config: schemas.SystemConfigCreate, db: Session = Depends(get_db)):
...
def create_product(product: schemas.ProductCatalogCreate, db: Session = Depends(get_db)):
