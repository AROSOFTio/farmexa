...
def create_record(record: schemas.SlaughterRecordCreate, db: Session = Depends(get_db)):
...
def update_record(record_id: int, updates: schemas.SlaughterRecordUpdate, db: Session = Depends(get_db)):
...
def add_output(record_id: int, output: schemas.SlaughterOutputCreate, db: Session = Depends(get_db)):
